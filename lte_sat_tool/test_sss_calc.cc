...
/////////sss_gen_c中产生的SSS序列
    int m0,m1;
        ...
        {
            sss_0_out[2*i]   = gr_complex (s_0_m0[i]*c_0[i] *128, 0);
            sss_10_out[2*i]   = gr_complex (s_1_m1[i]*c_0[i] *128, 0);
    ...
    for (int i = 0; i < 31; ++i)
    {
        even[i] = sss_0_out[5 + 2 * i + 0];
        odd[i]  = sss_0_out[5 + 2 * i + 1];
    ...
    xcorr(result,reice,ssss_ref,62);
    ...
    file1.close();
    std::cout<<"max_pos1="<<max_pos1<<";m1_ba="<<m1_ba<<";m1="<<m1<<std::endl;
